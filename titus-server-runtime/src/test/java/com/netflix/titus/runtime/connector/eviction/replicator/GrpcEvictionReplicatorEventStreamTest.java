...
public class GrpcEvictionReplicatorEventStreamTest {
    ...
    public void testJobQuotaUpdate() {
        ...
        newConnectVerifier()
                .assertNext(next -> assertThat(next.getData().findJobEvictionQuota("job1").get().getQuota()).isEqualTo(1))
                .then(() -> evictionComponentStub.setJobQuota("job1", 5))
                .thenConsumeWhile(next -> !isJobQuotaUpdate(next, "job1", 5))
                .assertNext(next -> assertThat(next.getData().findJobEvictionQuota("job1").get().getQuota()).isEqualTo(5))
                .thenCancel()
                .verify();
    }
    ...
    private boolean isJobQuotaUpdate(ReplicatorEvent<EvictionDataSnapshot> event, String jobId, int expectedJobQuota) {
        return event.getData().findJobEvictionQuota(jobId).map(q -> q.getQuota() == expectedJobQuota).orElse(false);
    }
}
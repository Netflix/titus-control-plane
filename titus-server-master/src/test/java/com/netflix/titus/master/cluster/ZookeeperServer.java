...
public class ZookeeperServer {
    ...
    private final static Logger LOG = Logger.getLogger(ZookeeperServer.class);
    ...
    public ZookeeperServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace) {
        this(dataDir, logDir, defaultNameSpace, DEFAULT_PORT);
    }

    public ZookeeperServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, int port) {
        this(dataDir, logDir, defaultNameSpace, port, DEFAULT_TICK_TIME);
    }

    public ZookeeperServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, int port, int tickTime) {
        this(dataDir, logDir, defaultNameSpace, port, tickTime, DEFAULT_MIN_SESSION_TIMEOUT);
    }

    public ZookeeperServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, int port, int tickTime, int minSessionTimeout) {
        this(null, dataDir, logDir, defaultNameSpace, port, tickTime, minSessionTimeout);
    }

    public ZookeeperServer(String hostName, String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, int port, int tickTime, int minSessionTimeout) {
        _hostName = hostName;
        _dataDir = dataDir;
        _logDir = logDir;
    ...
    @PostConstruct
    public void start() {
        final String[] localHostNames = _hostName == null
                ? NetworkUtil.getLocalHostNames()
                : new String[]{_hostName};
        String names = "";
        ...
        startZooKeeperServer(localHostNames);
        _zkClient = new ZkClient("localhost:" + _port, 10000);
        _defaultNameSpace.createDefaultNameSpace(_zkClient);
    }

    private void startZooKeeperServer(String[] localhostHostNames) {

...
class Batcher {
    ...
    Batcher(long timeoutMs, int batchSize, LoadBalancerConnector loadBalancerConnector, TargetTracking targetTracking, Scheduler scheduler) {
    ...
    private Observable<Batch> processBatch(Map<LoadBalancerTarget, LoadBalancerTarget.State> targetsWithState) {
        ...
        final List<Completable> registerAndUpdate = batch.getToRegister().byLoadBalancerId().entrySet().stream().map(entry -> {
            ...
            final Completable updateRegistered = targetTracking.updateTargets(targets.stream()
        ...
        final List<Completable> deregisterAndRemove = batch.getToDeregister().byLoadBalancerId().entrySet().stream().map(entry -> {
            ...
            return loadBalancerConnector.deregisterAll(loadBalancerId, ipAddresses)
                    .andThen(targetTracking.removeTargets(targets));
        }).collect(Collectors.toList());

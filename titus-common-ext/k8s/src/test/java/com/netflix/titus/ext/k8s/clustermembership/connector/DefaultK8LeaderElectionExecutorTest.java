...
public class DefaultK8LeaderElectionExecutorTest {
    ...
    private class MemberHolder {
        ...
        private final DefaultK8LeaderElectionExecutor executor;
        private final TitusRxSubscriber<LeaderElectionChangeEvent> eventSubscriber = new TitusRxSubscriber<>();

        MemberHolder() {
            this.memberId = newMemberId();
            this.executor = new DefaultK8LeaderElectionExecutor(
                    K8S_RESOURCE.getClient(),
                    "default",
                    clusterName,
                    LEASE_DURATION,
                    memberId,
        ...
        DefaultK8LeaderElectionExecutor getExecutor() {
            return executor;
        }

        LeaderElectionChangeEvent takeNextEvent() {
            try {
                return Preconditions.checkNotNull(eventSubscriber.takeNext(K8SExternalResource.K8S_TIMEOUT));

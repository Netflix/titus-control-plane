...
public class DefaultIndex<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> implements Index<OUTPUT> {
    ...
    DefaultIndex(IndexSpec<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> spec,
    ...
    DefaultIndex<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> newEmpty(IndexSpec<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> spec) {
        return new DefaultIndex<>(spec, new ListItemComparator<>(spec), HashTreePMap.empty(), TreePVector.empty());
    }
    ...
    DefaultIndex<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> add(Collection<INPUT> values) {
        ...
        for (INPUT input : filtered) {
            ...
            OUTPUT output = spec.getTransformer().apply(input);
        ...
        return new DefaultIndex<>(spec, listItemComparator, newPrimaryKeyToIndexKey, newSorted);
    }

    DefaultIndex<INDEX_KEY, PRIMARY_KEY, INPUT, OUTPUT> remove(Collection<PRIMARY_KEY> primaryKeys) {

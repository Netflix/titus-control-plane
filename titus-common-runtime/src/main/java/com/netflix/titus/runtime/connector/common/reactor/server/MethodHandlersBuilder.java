...
class MethodHandlersBuilder {
    ...
    MethodHandlersBuilder(Object reactorService,
        ...
        serviceDefinition.getMethods().forEach(methodDescriptor -> {
            ...
            if (binding.isMono()) {
                unaryMethodHandlers.add(new UnaryMethodHandler<>(binding, callMetadataResolver, reactorService));
            } else {
                serverStreamingMethodHandlers.add(new ServerStreamingMethodHandler<>(binding, callMetadataResolver, reactorService));
            }
        });
    ...
    private GrpcToReactorMethodBinding findReactorMethod(MethodDescriptor<?, ?> methodDescriptor) {
        ...
        if (reactorMethod.getParameterCount() == 1) {
            if (reactorMethod.getParameterTypes()[0] == CallMetadata.class) {
                Preconditions.checkArgument(
                        inputTypeName.equals(Empty.class.getSimpleName()),
                        "Only Empty request argument allowed for Reactor methods with no parameters: %s", methodDescriptor.getFullMethodName()
                );
        ...
        if (reactorMethod.getParameterCount() == 2) {
            Preconditions.checkArgument(
                    reactorMethod.getParameterTypes()[0] == CallMetadata.class || reactorMethod.getParameterTypes()[1] == CallMetadata.class,
            ...
            int callMetadataPos = reactorMethod.getParameterTypes()[0] == CallMetadata.class ? 0 : 1;
